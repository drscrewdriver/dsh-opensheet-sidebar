...
.PARAMETER NewSpec
  What to install: a registry spec (`dsh-opensheet-sidebar@1.0.0`), a git spec
  (`github:owner/repo#<sha>`), or a local path.

.PARAMETER OldName
  The package name to remove from the profile (default: dsh-csv-sidebar).
...
.EXAMPLE
  pwsh -File scripts/migrate-profile.ps1 -Profile web -NewSpec 'github:drscrewdriver/dsh-opensheet-sidebar#31a5a3f'
#>
[CmdletBinding()]
param(
...
$oldDir = Test-Path (Join-Path (Join-Path $profileDir 'node_modules') $OldName)
$newDir = Test-Path (Join-Path (Join-Path $profileDir 'node_modules') $newName)

Write-Host ("  dependencies : old={0} new={1}" -f $depsHasOld, $depsHasNew)
Write-Host ("  bundles      : old={0} new={1} (count {2})" -f $bundlesHasOld, $bundlesHasNew, $bundles.Count)
Write-Host ("  node_modules : old={0} new={1}" -f $oldDir, $newDir)

if ($depsHasOld) { $failures.Add("dependencies still lists $OldName") }
